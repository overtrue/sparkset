...
export class HeaderAuthProvider implements AuthProvider {
  ...
  async authenticate(ctx: HttpContext): Promise<User | null> {
    ...
    try {
      ...
      const userData = {
        ...
        permissions: this.parsePermissions(ctx.request.header(`${this.getHeaderPrefix()}Permissions`)),
      }
  ...
  private isIpInCidr(ip: string, cidr: string): boolean {
    ...
    if (bitNum === 24) return ipParts[0] === rangeParts[0] && ipParts[1] === rangeParts[1] && ipParts[2] === rangeParts[2]
  ...
  private parseRoles(header: string | null): string[] {
    if (!header) return []
    return header.split(',').map(s => s.trim()).filter(s => s)
  }
